...
const EditAlbum = () => {
  ...
  const filterAlbums = () => {
    return albums.filter((album) => {
      ...
      const artists = album.artists ? album.artists.join(', ').toLowerCase() : '';
      const genre = album.genre ? album.genre.toLowerCase() : '';
  
      return (
        albumName.includes(searchQuery.toLowerCase()) ||
        artists.includes(searchQuery.toLowerCase()) ||
